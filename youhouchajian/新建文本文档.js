...
// @version      4.3.9
// @author       oolikeABC
// @description  💯自动刷课【🤖AI强补】题库盲区自动触发AI答题，精准无死角；【🚀极速全能】视频音频倍速秒过，文档图书自动完成，支持字体解密；【🥇全程托管】一键安装即用，自动切换任务点，作业考试自动答题提交；【📔顶配题库】整合多方题库每日更新，开放自定义参数，功能持续进化。
// @license      Proprietary
...
(function (vue, Antd, pinia, marked, rxjs, md5, DOMPurify) {
  ...
  const aiSmartLogic = () => {
    ...
    const watchUrlChange = () => {
      ...
      setInterval(() => {
        if (location.href === prevUrl) return;
        prevUrl = location.href;
        if (location.href.includes("fusioncourseh5.zhihuishu.com") || location.href.includes("smartcoursestudent.zhihuishu.com")) {
          log.insertLog("检测到页面切换，重新初始化..");
  ...
  const zhsRoutes = {
    match: [
      ...
      { keyword: "smartcoursestudent.zhihuishu.com", tab: "1", logic: aiSmartLogic },
