...
function openWindowUrl(pageHeader, url, module) {
...
$(document).ready(function () {
    ...
    //动态为该元素添加章节的样式  等同于.nav-content ul li a i.zj
    $('.nav-content ul li a i.MyChapters').css('background-position', '-48px 0');
    ...
    // 课程内自定义的课程承诺书 0：不同意 1：同意 -1 未开启
    var notAgreeCourseCommitment = $("#notAgreeCourseCommitment").val();

    if((notAgreeCommitment == "true" && notAgreeCourseCommitment == "-1") || notAgreeCourseCommitment == "0") {
        showCommitmentDiv();
    }else {
        showCourseNote();
    }
    ...
    $(".nav-content li,.into_ItemBank").click(function () {
        ...
        var knowledgeEnc = $("#knowledgeEnc").val();
        var topicModelId = $("#topicModelId").val();
        var taskEngine = $("#taskEngine").val();
        ...
        var showAiAssistantTea =  $("#showAiAssistantTea").val() || false;
        var showAIEnc = $("#showAIEnc").val();
	    ...
	    if(module == "zy" || module == "zb_jm" || module == "bjexpand"){
            ...
            url = ServerHost.mooc1Domain + url + "chapterId="+chapterId+"&courseId="+courseid+"&clazzid="+clazzid+"&cpi="+cpi+"&mooc2=1&enc="+oldenc+"&microTopicId="+microTopicId + "&topicModelId=" + topicModelId + "&taskEngine=" + taskEngine;
        }else if(module == "WK_TJ") {
            url = url + "courseid=" + courseid + "&cpi="+cpi+"&clazzid=" + clazzid + "&ut=s&topicId=" + microTopicId;
        }else if(module == "LIBRARY_RE") {
            var param = '';
            var isPreviewModel = $("#isPreviewModel").val();
            if(typeof isPreviewModel != "undefined" && isPreviewModel == "1") {
                var sourceEnc = $("#sourceEnc").val();
                var sourceCpi = $("#sourceCpi").val();
                param = "courseid="+courseid+"&cpi="+sourceCpi+"&ut=t&microTopicId="+microTopicId+"&enc="+sourceEnc + "&isPreviewModel=1";
            }else {
                param = "courseid="+courseid+"&clazzid="+clazzid+"&cpi="+cpi+"&ut=s&microTopicId="+microTopicId;
            }
            var openurl = "/mooc2-ans/coursetopic/library-embed?" + param;
            openLibraryUrl(openurl, module);
            return;//到这里可以直接返回了，否则异步调用会让页面二次加载
        } else if (module == "microTJZY") {
            url = url + "courseid=" + courseid + "&clazzid=" + clazzid + "&cpi=" + cpi + "&ut=s&microTopicId=" + microTopicId;
        } else if (module == "ai_workbench") {
            //nothing
        }else if(module == 'zc-wk') {
            url += "courseId=" + courseid + "&classId=" + clazzid + "&cpi=" + cpi + "&ut=s&topicid=" + microTopicId ;
        }else if(module == 'wk-preview') {
            url += "courseId=" + courseid + "&knowledgeId=" + chapterId;
        }else if (module == 'MyChapters'){
            url += "courseId=" + courseid;
        }else if (module == "AIexperiment") {
            url += "courseId=" + courseid + "&classId=" + clazzid + "&cpi=" + cpi + "&enc=" + enc + "&t=" + t + "&ut=s";
        }else{
	        url += "courseid=" + courseid + "&clazzid=" + clazzid + "&cpi=" + cpi + "&ut=s&t="+t;
	    }

        //加密
        if(url.indexOf("?cpi=")==-1 && url.indexOf("&cpi=")==-1){
            url += "&cpi=" +cpi;
        }
        ...
        try {
            var isMicroNodeUrl = $(this).attr("isMicroNodeUrl");
            if(typeof isMicroNodeUrl != "undefined" && isMicroNodeUrl) {
                if(module == "zy") {
                    url += "&topicId=" + microTopicId;
                }else if(module == "ks") {
                    url += "&topicid=" + microTopicId;
                }
            }
        ...
        try {
            ...
            if(parseInt(microCourseTopicOpenType) == 1 && (module == "tl" || module == "microTL")){
        ...
        if (module == 'tl' || module == 'microTL') {
            pageHeader = 2;
            if(courseApp == 2 && bbsUrlSwitch == "false" && module != 'microTL'){
            ...
            url = url + "&showAIEnc=" + showAIEnc + "&supportAI=" + showAiAssistantTea;
            ...
            if(module == 'microTL'){
        		url += "&knowledgeEnc=" + knowledgeEnc + "&chapterId="+chapterId;
        	}
        }
        if (module == 'zl') {
            pageHeader = 3;
        ...
        if (module == 'labreport') {
            var isMapCourse = $("#isMapCourse").val();
            var isRelation = 1;
            if(isMapCourse == "true"){
                isRelation = 1;
            }
            url +=  "&isRelation="+isRelation;
            pageHeader = 23;
        }
        ...
        if (module == "MyChapters") {
            pageHeader = 1;
        }
        if (module == "AIexperiment") {
            pageHeader = 25;
        }

        // 检查是否需要重定向到中间页
        var needRedirect = $(this).attr("needRedirect") || 1;
        ...
        openWindowUrl(pageHeader, url, module);
    ...
    var updateTipHead = $("#updateTipHead").val();
    // 创建新元素
    var $noiceDiv = $('<div>', {
        'class': 'marktipBtn marRight30 fl',
        'style': 'display:none',
        html: '<i></i>'+updateTipHead+'<span>查看详情</span>'
    });
    // 在第一个smarkcoursemark元素前插入
    $noiceDiv.insertBefore($('.smarkcoursemark').first());
    // 是否开启更新提示
    var openUpdateTip = $("#openUpdateTip").val();
    if(openUpdateTip === 'true'){
        $noiceDiv.show();
    }

    // 绑定元素 点击详情 展示弹窗
    $(".marktipBtn span").click(function (){
        $(".markPopup").show();
    })
    // 点击我知道了  隐藏弹窗以及通知
...
function openLibraryUrl(url, module){
    $.ajax({
        url : url,
        type : "get",
        dataType : "json",
        success : function(data){
            if(data.status){
                ...
                if(data.openType == 1){
                    ...
                    openWindowUrl(20, openUrl, module);
                }
            }
        }
    });
...
$("#showCommitmentId").on("click",".commitSure",function(){
...
function isAccessibleCustomFidClickNav(thisDiv) {
    ...
    var hideHead =$("hideGHead").val();
    if (module=="hd"&&hideHead=='1'){
      $('#frame_content-hd').css('inset', '0px 0px 0px 180px');
    }
}
...
function copyPortalUrl() {
    var portalUrl = $("#tpsubmit").attr("action");
    if (!portalUrl) {
        return;
    }
    try {
        ...
        copyInput.setAttribute('value', portalUrl);
        copyInput.select();
        document.execCommand("Copy");
        copyInput.remove();
        $.toast({type: 'success', content: I18N_Config.successTip});
    } catch (err) {
